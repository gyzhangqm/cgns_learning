...
int main(int argc, char *argv[])
{
	...
	strcpy(data.fileName, "/home/guesser/cgns_examples/output/UnstructuredGrid2D_mixed.cgns");
	strcpy(data.baseName, "Simple structured grid");
	strcpy(data.zoneName, "Grid");
	strcpy(data.gridConnectivitySectionName, "conectivity");
	...
	generateBase(&data);
	generateZone2D_mixed(&data);
	generateCoordinates2D(&data);
	generateElementsConnectivity2D_mixed(&data);
